...
const BENGALI_MONTHS: Record<string, number> = {
  'জানুয়ারি': 1, 'জানুয়ারি': 1, 'january': 1, 'jan': 1,
  'ফেব্রুয়ারি': 2, 'ফেব্রুয়ারি': 2, 'february': 2, 'feb': 2,
...
/**
 * Parses spoken dates in Bengali or English to YYYY-MM-DD format.
 * Examples: "১২ এপ্রিল ১৯৯৫", "1995-05-20", "15/08/2000", "2002"
 */
export const parseSpokenDate = (text: string): string => {
  if (!text) return '';
  const normalized = normalizeBengaliDigits(text.trim());
  ...
  // Look for Bengali or English month names
  let foundMonth: number | null = null;
  const lower = normalized.toLowerCase();
  for (const [mName, mIdx] of Object.entries(BENGALI_MONTHS)) {
  ...
  if (foundMonth !== null) {
    ...
    if (numbers && numbers.length >= 2) {
      ...
      return `${year}-${String(foundMonth).padStart(2, '0')}-${String(day).padStart(2, '0')}`;
    }
  }
