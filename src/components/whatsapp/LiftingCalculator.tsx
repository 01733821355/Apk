              ...
              <div key={item.id} className="flex items-center gap-2">
                <div className="relative flex-1">
                  <input
                    ...
                    inputMode="decimal"
                    placeholder="0.00"
                    className={`w-full rounded-2xl p-3.5 sm:p-4 pr-11 focus:ring-2 focus:ring-emerald-400 text-base sm:text-lg border ${isDark ? 'bg-slate-900 border-slate-700 text-white' : 'bg-slate-50 border-slate-200 text-slate-900'}`}
                  ...
                  <div className="absolute right-3 top-1/2 -translate-y-1/2">
                    <VoiceInputButton
                      inputType="number"
                      contextLabel="সেলস টাকা"
                      onTranscript={(val) => updateItem('sales', item.id, val)}
                      size="xs"
                    />
                  </div>
                </div>
              ...
              <div key={item.id} className="flex items-center gap-2">
                <div className="relative flex-1">
                  <input
                    ...
                    inputMode="decimal"
                    placeholder="0.00"
                    className={`w-full rounded-2xl p-3.5 sm:p-4 pr-11 focus:ring-2 focus:ring-rose-400 text-base sm:text-lg border ${isDark ? 'bg-slate-900 border-slate-700 text-white' : 'bg-slate-50 border-slate-200 text-slate-900'}`}
                    value={item.value === 0 ? '' : item.value}
                  ...
                  <div className="absolute right-3 top-1/2 -translate-y-1/2">
                    <VoiceInputButton
                      inputType="number"
                      contextLabel="ক্যান্সেল টাকা"
                      onTranscript={(val) => updateItem('cancel', item.id, val)}
                      size="xs"
                    />
                  </div>
                </div>
