        ...
        <form onSubmit={handleDelete} className="space-y-4">
          <div>
            <div className="flex items-center justify-between mb-1.5">
              ...
              <VoiceInputButton
                inputType="text"
                contextLabel="DELETE বা ডিলিট বলুন"
                onTranscript={(val) => {
                  const cleaned = val.trim().toLowerCase();
                  if (cleaned.includes('del') || cleaned.includes('ডিলিট') || cleaned.includes('মুছে')) {
                    setConfirmInput('DELETE');
                  } else {
                    setConfirmInput(val.toUpperCase());
                  }
                }}
                size="xs"
              />
            ...
            <div className="relative">
              <input
                ...
                className="w-full px-10 py-2 text-sm font-mono font-bold bg-white border-2 border-slate-300 rounded-xl focus:border-rose-500 focus:ring-2 focus:ring-rose-500/20 text-center tracking-widest uppercase"
                autoFocus
              />
              <div className="absolute right-2 top-1/2 -translate-y-1/2">
                <VoiceInputButton
                  inputType="text"
                  contextLabel="DELETE বলুন"
                  onTranscript={(val) => {
                    const cleaned = val.trim().toLowerCase();
                    if (cleaned.includes('del') || cleaned.includes('ডিলিট') || cleaned.includes('মুছে')) {
                      setConfirmInput('DELETE');
